...
async function getGoogleAccessToken(): Promise<string> {
  // Recargar el .env.local para obtener el token más actualizado
  dotenv.config({ path: '.env.local', override: true });
  const refreshToken = process.env.GOOGLE_REFRESH_TOKEN;

  if (!refreshToken) {
    throw new Error(
      'Google Calendar no está vinculado. Por favor el dueño debe visitar http://localhost:3001/api/auth para autenticarse primero.'
    );
  }
...
app.get('/api/auth/callback', async (req, res) => {
  ...
  try {
    ...
    // Guardar el refresh_token en .env.local y en el proceso actual
...
app.post('/api/create-booking', async (req, res) => {
  try {
    ...
    if (!gcalRes.ok) {
      const errData = await gcalRes.json() as any;
